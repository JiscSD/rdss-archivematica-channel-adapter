...
# Create SQS queue
aws 4576 sqs delete-queue \
	--queue-url="http://localhost:4576/queue/main" || true 2>/dev/null
aws 4576 sqs create-queue \
	--queue-name="main" --attributes "VisibilityTimeout=600"

# Create SNS topics
aws 4575 sns delete-topic \
	--topic-arn="arn:aws:sns:us-east-1:123456789012:main" || true 2>/dev/null
aws 4575 sns create-topic \
	--name="main"
aws 4575 sns delete-topic \
	--topic-arn="arn:aws:sns:us-east-1:123456789012:invalid" || true 2>/dev/null
aws 4575 sns create-topic \
	--name="invalid"
aws 4575 sns delete-topic \
	--topic-arn="arn:aws:sns:us-east-1:123456789012:error" || true 2>/dev/null
aws 4575 sns create-topic \
	--name="error"

# Create DynamoDB tables
aws 4569 dynamodb delete-table \
	--table-name="rdss_archivematica_adapter_local_data_repository" || true 2>/dev/null
aws 4569 dynamodb create-table \
